...
const AdminSideBar = () => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-40 w-64 max-sm:w-screen h-screen transition-transform duration-300 ${open? 'translate-x-0':'-translate-x-full'}  sm:translate-x-0`}
        aria-label='Sidebar'>
        <div className='h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800'>
          <div className='flex items-center ps-2.5 mb-5'>
            <img src={session?.user?.image ?? ''} className='h-7 me-3 sm:h-8 rounded-full' alt='Admin Logo' />
            <span
              className='self-center text-xl font-semibold whitespace-nowrap dark:text-white'>{session?.user?.name}</span>
