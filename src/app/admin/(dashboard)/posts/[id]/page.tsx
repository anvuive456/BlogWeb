...
const Page = ({ params }: { params: { id: number } }) => {
  ...
  const getPost = async () => {
    const result = await fetch(baseApiUrl + `/admin/posts/${params.id}`).then(res => res.json());
    if (!result.post) notFound();
    const { categoryId, description, title, content, image } = result.post as PostFullOptions;
    console.log('post category id', categoryId);
  ...
  const submit = async (form: FormData) => {
    ...
    const result = await fetch(baseApiUrl + `/admin/posts/${params.id}`, {
      method: 'PUT',
      body: form,
      ...
      <div className='p-4 bg-blue-50 h-screen overflow-y-scroll'>
        <form action={submit}>
          ...
          <div className='flex flex-wrap mb-2 gap-2'>
            <select value={selectedCate} onChange={(e) => setSelectedCate(Number(e.target.value))}
