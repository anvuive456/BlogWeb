...
type ArticleCardElement = JSX.Element;
export default function LoadMore({slug}: { slug: string }) {
  const {ref, inView} = useInView();
  const [data, setData] = useState<ArticleCardElement[]>([]);
  // const searchParams = useSearchParams();

  useEffect(() => {
    if (inView) {
      getCategoryPosts({slug, page: page}).then(value => {
        setData([...data,...value]);
        page++;
      });
    }
  ...
  return (
      <>
        ...
        <div className='w-full flex flex-row justify-center' ref={ref} role="status">
